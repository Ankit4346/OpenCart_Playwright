...
- Name: LoginDataDriven.spec.ts >> Login using JSON Data : Invalid login
- Location: tests\LoginDataDriven.spec.ts:35:7

# Error details

```
TypeError: data.expected.toLower is not a function
```
...
```ts
  ...
  25  | 
  26  | const jsonPath='data/logindata.json';
  27  | let jsonData:any=DataProvider.getTestDataFromJson(jsonPath);
  28  | 
  29  | const csvPath='data/logindata.csv';
  30  | let csvData:any=DataProvider.getTestDataFromCsv(csvPath);
  31  | 
  32  | // ============================== JSON Data DRiven =====================================
  33  | for (let data of jsonData){
  34  | 
  35  |   test(`Login using JSON Data : ${data.testName}`,async ({page})=>{
  36  | 
  37  | config=new TestConfig();
  38  | 
  39  | await page.goto(config.appUrl);
  40  | 
  41  | homePage=new HomePage(page);
  42  | loginPage=new LoginPage(page);
  43  | myAccountPage=new MyAccountPage(page);
  44  | 
  45  | await homePage.clickMyAccount();
  46  | await homePage.clickLogin();
  47  | await loginPage.setEmail(data.email);
  48  | await loginPage.setPassword(data.password);
  49  | await loginPage.clickLogin();
  50  | 
> 51  | if (data.expected.toLower()==='success'){
      |                   ^ TypeError: data.expected.toLower is not a function
  52  | 
  53  |   const isLoggedIn=await myAccountPage.isMyAccountPageExists();
  54  |   expect(isLoggedIn).toBeTruthy();
  55  |   console.log('Account Logged in Successfully');
  56  | }
  57  | 
  58  | else{
  59  | 
  60  | const errorMsg=loginPage.getloginErrorMessage();
  61  | expect(errorMsg).toContain('');
  62  |   console.log('Account NOT logged ');
  63  | }
  64  | 
  65  |   });
  66  | 
  ...
  69  | 
  70  | // ============================== CSV Data Driven =====================================
  71  | for (let data of csvData){
  72  | 
  73  |   test(`Login using CSV Data : ${data.testName}`,async ({page})=>{
  74  | 
  75  | config=new TestConfig();
  76  | 
  77  | await page.goto(config.appUrl);
  78  | 
  79  | homePage=new HomePage(page);
  80  | loginPage=new LoginPage(page);
  81  | myAccountPage=new MyAccountPage(page);
  82  | 
  83  | await homePage.clickMyAccount();
  84  | await homePage.clickLogin();
  85  | await loginPage.setEmail(data.email);
  86  | await loginPage.setPassword(data.password);
  87  | await loginPage.clickLogin();
  88  | 
  89  | if (data.expected.toLower()==='success'){
  90  | 
  91  |   const isLoggedIn=await myAccountPage.isMyAccountPageExists();
  92  |   expect(isLoggedIn).toBeTruthy();
  93  |   console.log('Account Logged in Successfully');
  94  | }
  95  | 
  96  | else{
  97  | 
  98  | const errorMsg=loginPage.getloginErrorMessage();
  99  | expect(errorMsg).toContain('Warning: No match');
  100 |   console.log('Account NOT logged ');
  101 | }
  102 | 
  103 |   });
  104 | 
